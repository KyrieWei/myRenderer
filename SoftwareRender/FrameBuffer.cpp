...
void FrameBuffer::draw_filled_triangle(const vec2& point0, const vec2& point1, const vec2& point2, const vec4& color)
{
	...
	int delta_y_01 = tmp_1.y - tmp_0.y;
	int delta_y_02 = tmp_2.y - tmp_0.y;
	int delta_y_12 = tmp_2.y - tmp_1.y;
	int delta_x_01 = tmp_0.x - tmp_1.x;
	int delta_x_02 = tmp_0.x - tmp_2.x;
	int delta_x_12 = tmp_1.x - tmp_2.x;

	//draw the down half triangle
	for (int i = tmp_0.y; i <= tmp_1.y; i++)
	{
		int x0 = static_cast<int>((double)(i - tmp_0.y) / delta_y_01 * delta_x_01 + tmp_0.x);
		int x1 = static_cast<int>((double)(i - tmp_0.y) / delta_y_02 * delta_x_02 + tmp_2.x);
	...
	{
		int x0 = static_cast<int>((double)(tmp_2.y - i) / delta_y_12 * delta_x_12 + tmp_1.x);
		int x1 = static_cast<int>((double)(tmp_2.y - i) / delta_y_02 * delta_x_02 + tmp_0.x);
